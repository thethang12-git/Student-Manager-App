...
export default function ActionBar ({isOpen}:{isOpen:boolean}) {
    ...
    useEffect(() => {
        if(actionType == 'calendar') setDisplay(false)
        else setDisplay(true);
    },[actionType])
    return (
        <div style={{display:isDisplay? 'flex' : 'none'}} className="bg-white p-4 rounded-xl shadow-md mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-3 sm:space-y-0">
